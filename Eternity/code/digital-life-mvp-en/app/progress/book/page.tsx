...
const FALLBACK_CHAPTER = 'Uncategorized'
...
export default function BookProgressPage() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        if (!user) {
          if (!canceled) setError('Please log in to view book progress')
          return
        }
        ...
        if (!canceled) setError(e?.message ?? 'Failed to load Account')
      } finally {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (!canceled) setError(e?.message ?? 'Failed to load progress')
      } finally {
        if (!canceled) setLoading(false)
      }
    }
  ...
  const cards = useMemo(() => {
    ...
    return answers.slice(0, 12).map((a) => {
      ...
      const title = q?.text || 'Untitled question'
      const summary = a.transcript_text?.slice(0, 120) || 'Transcription in progress, summary/quote cards coming soon...'
      return {
        id: a.id,
        title,
        chapter,
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-10">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-bold text-slate-900">Book Progress and Instant Cards</h1>
            <p className="mt-1 text-sm text-slate-600">Cards generated after recording: Summary / Quotes / Timeline. Check chapter progress anytime.</p>
          </div>
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <StatTile label="Recorded answers" value={`${answers.length}`} hint="Cards generated after recording" />
          <StatTile label="Chapters covered" value={`${chapterStats.length}`} hint="Chapter has at least one answer" />
          <StatTile label="Latest outline" value={latestOutline ? `v${latestOutline.version} · ${latestOutline.status}` : 'None'} hint={latestOutline ? new Date(latestOutline.created_at).toLocaleString() : 'View after generating outline'} />
        </section>
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:col-span-2">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-lg font-semibold text-slate-900">Chapter Completion</h3>
              </div>
              <Link href="/progress" className="text-sm font-semibold text-amber-600 hover:text-amber-700">View chapter path</Link>
            </div>
            {chapterStats.length === 0 ? (
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">No chapter data yet. Answer some questions first.</div>
            ) : (
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {chapterStats.map((c) => (
                  <div key={c.name} className="rounded-xl border border-slate-200 bg-white/70 p-3 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
                    ...
                    <div className="mt-1 text-xs text-slate-500">{c.done} / {c.total} questions</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-3">
              ...
              <h3 className="text-lg font-semibold text-slate-900">Final Preview</h3>
            </div>
            <div className="space-y-3 text-sm text-slate-700">
              <Link href="/stories" className="block rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 font-semibold text-amber-700 hover:border-amber-200 hover:bg-amber-50">Story Scroll Preview</Link>
              <Link href="/export" className="block rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 font-semibold text-slate-800 hover:border-emerald-200 hover:bg-emerald-50">E-book Export/PDF</Link>
              <div className="rounded-lg border border-dashed border-slate-300 px-3 py-2 text-slate-500">Hardcover/Photo book (Pre-order)</div>
              <div className="rounded-lg border border-dashed border-slate-300 px-3 py-2 text-slate-500">Future letter/Memorial page (Single page)</div>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-lg font-semibold text-slate-900">Cards on completion</h3>
            </div>
            <span className="text-xs font-semibold text-amber-600">Latest 12</span>
          </div>
          {cards.length === 0 ? (
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">No recording cards yet. Try recording one.</div>
          ) : (
              ...
              {cards.map((card) => (
                <div key={card.id} className="relative overflow-hidden rounded-xl border border-slate-200 bg-gradient-to-b from-white to-slate-50 p-3 shadow-[0_12px_36px_rgba(15,23,42,0.06)]">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-semibold text-amber-700">{card.status === 'done' ? 'Generated' : 'Generating'}</span>
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-between text-[11px] text-slate-500">
                    ...
                    <Link href="/export" className="font-semibold text-amber-700 hover:text-amber-800">Insert into manuscript</Link>
                  </div>
                </div>
              ))}
            </div>
