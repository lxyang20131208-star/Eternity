  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
              {question.chapter || 'Uncategorized'}
            </div>
      ...
      <div className="mt-4 flex gap-3">
        ...
        {question.status === 'completed' && (
          <div className="flex-1 text-center text-sm font-semibold text-emerald-700">✓ Completed</div>
        )}
        {question.status === 'skipped' && (
          <div className="flex-1 text-center text-sm font-semibold text-slate-500">Skipped</div>
        )}
      </div>
    </div>
  )
...
export default function TodayPage() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        if (!user) {
          if (!canceled) setError('Please log in to view daily questions')
          return
        }
        if (!canceled) {
          setUserId(user.id)
  ...
  useEffect(() => {
    ...
    async function loadReminder() {
      ...
      try {
        ...
        const msg = e?.message || 'Cannot read reminder settings, please ensure latest database migrations are applied'
        showToast(msg)
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (!canceled) setError(e?.message ?? 'Failed to load questions')
      } finally {
        if (!canceled) setLoading(false)
      }
    }
  ...
  function handleSkip(qid: string) {
    ...
    showToast('Question skipped, will show again tomorrow')
  }
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-8 px-4 py-10">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-bold text-slate-900">Daily 3 Questions</h1>
            <p className="mt-1 text-sm text-slate-600">Just 3 minutes a day to record your life stories.</p>
          </div>
          <div className="flex items-center gap-3">
          ...
          <>
            <div className="rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-white p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-amber-800">Today's Progress</div>
                  <div className="mt-1 text-xs text-amber-700">
                    {allDone ? '🎉 All daily questions completed！' : `Completed ${completedCount} / ${todayQuestions.length}  questions`}
                  </div>
                </div>
            ...
            {todayQuestions.length === 0 && (
              <div className="rounded-xl border border-slate-200 bg-white px-4 py-6 text-center text-slate-500">
                No questions available. Please create questions in the question bank first.
              </div>
            )}
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-slate-700">📬 Weekly Email Reminders</div>
                  <div className="mt-1 text-xs text-slate-600">Set up email reminders to never miss your daily questions</div>
                </div>
                <button
                  onClick={async () => {
                    if (!userId) {
                ...
                >
                  {reminderEnabled ? 'Enabled' : reminderLoading ? 'Enabling...' : 'Enable Email Reminders'}
                </button>
              </div>
            </div>
          </>
