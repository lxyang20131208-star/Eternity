...
export default function OutlineEditPage() {
  ...
  const handleSaveAsNew = async () => {
    ...
    if (result.success) {
      setIsDirty(false)
      showToast(`Saved as new version v${result.version}`, 'success')
    ...
    return (
      <div className="detroit-bg" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <p style={{ marginBottom: 16, color: 'var(--text-secondary)' }}>Please log in first</p>
          <Link href="/" className="cyber-btn">Back to Home</Link>
        </div>
      </div>
    )
  }
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>
            Edit, adjust and polish your biography outline
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: 20 }}>
        ...
        <div className="glass-card" style={{ padding: 16, height: 'fit-content', maxHeight: 'calc(100vh - 140px)', overflowY: 'auto' }}>
          ...
          {outlines.map((o) => (
            ...
            >
              <div style={{ fontWeight: 600 }}>Version {o.version}</div>
          ...
          {editedContent ? (
            ...
            <div className="glass-card" style={{ padding: 40, textAlign: 'center' }}>
              <p style={{ color: 'var(--text-muted)' }}>
                {outlines.length > 0 ? 'Select a version to start editing' : 'No outline data'}
              </p>
            </div>
          )}
        </div>
