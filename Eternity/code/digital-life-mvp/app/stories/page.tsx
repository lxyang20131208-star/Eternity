...
export default function StoriesPage() {
  ...
  useEffect(() => {
    ...
    async function loadOutlines() {
      ...
      try {
        ...
        if (!canceled) setError(err?.message || '加载大纲失败')
      } finally {
  ...
  const story = useMemo<Story>(() => {
    if (!latestOutline || !latestOutline.outline_json) {
      return {
        title: '尚未生成故事',
        tagline: '去大纲标注为章节附上照片与备注，自动生成故事卷',
        era: '时间待补充',
        location: '地点待补充',
    ...
    const sectionBlocks: StoryBlock[] = outline.sections.map((section, idx) => {
      ...
      const body = section.bullets[0] || '这一段等待你的补充。'
      const detail = primaryAtt?.note || section.bullets[1] || '添加备注，让故事更具体。'
      ...
      return {
        ...
        image,
        quote,
        tag: tagParts.join(' · ') || '未标注',
      }
    })

    const allDates = photos.map((p) => p.scene.date).filter(Boolean)
    const era = allDates.length > 0 ? `${allDates[0]} - ${allDates[allDates.length - 1]}` : '时间待补充'
    ...
    const location = Object.entries(topLocation).sort((a, b) => b[1] - a[1])[0]?.[0] || '地点待补充'
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto' }} className="story-shell">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, marginBottom: 18, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button
              ...
              onClick={handleShare}
            >
              {shareState === 'copied' ? '链接已复制' : shareState === 'shared' ? '已分享' : '一键分享/复制'}
            </button>
            <a
              className="cyber-btn"
              style={{ borderRadius: 6, padding: '10px 16px', fontSize: 13, textDecoration: 'none' }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, position: 'relative', zIndex: 1 }}>
            ...
            <div className="story-meta-grid">
              <div className="glass-card" style={{ padding: 14 }}>
                <div style={{ color: 'var(--text-secondary)', fontSize: 12 }}>时间</div>
                <div style={{ fontWeight: 600 }}>{story.era}</div>
              </div>
              <div className="glass-card" style={{ padding: 14 }}>
                <div style={{ color: 'var(--text-secondary)', fontSize: 12 }}>地点</div>
                <div style={{ fontWeight: 600 }}>{story.location}</div>
              </div>
              <div className="glass-card" style={{ padding: 14 }}>
                <div style={{ color: 'var(--text-secondary)', fontSize: 12 }}>人物 · 场景 · 照片</div>
                <div style={{ fontWeight: 600 }}>{story.stats.people} 人 / {story.stats.scenes} 场景 / {story.stats.photos} 张</div>
              </div>
              <div className="glass-card" style={{ padding: 14 }}>
                <div style={{ color: 'var(--text-secondary)', fontSize: 12 }}>主题</div>
            ...
            <div className="story-scroll-indicator" style={{ marginTop: 10 }}>向下滚动 · 开始浏览</div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 280px', gap: 20, marginTop: 18 }}>
          <div className="story-track">
            ...
            {!loading && !error && story.blocks.length === 0 && (
              <div className="glass-card" style={{ padding: 16, marginBottom: 12, borderColor: 'var(--accent-cyan)' }}>
                <div style={{ fontWeight: 700, marginBottom: 6 }}>还没有故事章节</div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                  去“照片标注”和“大纲标注”把照片附到章节，就能在这里生成故事卷轴。
                </div>
                <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                  <a className="cyber-btn" style={{ borderRadius: 6, padding: '8px 12px', fontSize: 12, textDecoration: 'none' }} href="/photos/new">标注照片</a>
                  <a className="cyber-btn" style={{ borderRadius: 6, padding: '8px 12px', fontSize: 12, textDecoration: 'none' }} href="/outline-annotate">大纲标注</a>
                </div>
              </div>
            )}
          ...
          <aside className="story-floating-actions">
            ...
            <div className="story-floating-card">
              <strong>素材绑定</strong>
              <small>后续将与人物/场景标签联动，点击节点直达照片集。</small>
            ...
            <div className="story-floating-card">
              <strong>导出计划</strong>
              <small>即将支持生成海报卡、EPUB/PDF 预排版。</small>
              <div style={{ display: 'grid', gap: 8, marginTop: 10 }}>
                <button className="cyber-btn" style={{ borderRadius: 6, padding: '9px 10px', fontSize: 12 }}>生成分享卡 (即将)</button>
                <button className="cyber-btn" style={{ borderRadius: 6, padding: '9px 10px', fontSize: 12 }}>EPUB / PDF (即将)</button>
              </div>
            </div>
          </aside>
        </div>
        ...
        <div id="create" className="glass-card" style={{ marginTop: 32, padding: 20, borderRadius: 10, borderColor: 'var(--accent-cyan)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <div>
              <div className="story-pill" style={{ marginBottom: 6 }}>下一步</div>
              <h3 style={{ margin: 0 }}>创建你的家族故事</h3>
              <p style={{ margin: '6px 0 0', color: 'var(--text-secondary)' }}>上传照片，选人物/场景标签，生成可转发的卷轴式页面。</p>
            </div>
