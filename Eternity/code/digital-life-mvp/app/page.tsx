...
export default function Home() {
  ...
  async function deleteFreeQuestion(questionId: string): Promise<void> {
    try {
      ...
      showToast('自由问题已删除', 'success')
    } catch (e: any) {
      console.error('删除自由问题失败:', e)
  ...
  const buildFollowUps = (text: string, chapter?: string | null): string[] => {
    ...
    if (/when|时间|时候|年代|年份|年龄/.test(t)) add('可以写出具体年份、年龄或大致时间段')
    if (/where|哪里|哪儿|地点|住在|城市|家乡|学校|公司/.test(t)) add('描述发生地点：城市、街区、房子或环境细节')
    if (/who|谁|人物|家人|同学|朋友|同事|老师/.test(t)) add('提到涉及到的人：他们是谁、关系怎样、说过什么')
    if (/why|原因|动机|怎么会|如何发生/.test(t)) add('说说背后的原因或动机，为什么会这样做/发生')
    if (/感受|感觉|心情|开心|难过|紧张|兴奋/.test(t)) add('补充当时的情绪：开心、紧张还是担心？')
    if (/结果|影响|收获|教训|改变/.test(t)) add('这件事带来了什么结果或改变？有没有收获或教训？')
    if (/第一次|初次/.test(t)) add('回忆第一次的细节：谁在场、场景、对话')
    if (/搬家|转学|变动|变故/.test(t)) add('说明变动的原因、搬去哪里、对生活的影响')
    if (/游戏|玩具|爱好|兴趣/.test(t)) add('举例当时最喜欢的玩具/游戏/爱好以及陪你的人')
    if (/老师|课程|学校|学习|考试|科目/.test(t)) add('提到印象深刻的老师、课程或一次考试/作业细节')
    if (/工作|职业|岗位|项目|同事|老板/.test(t)) add('描述岗位、团队、挑战，以及你如何解决问题')
    if (/家庭|父母|兄弟|姐妹|婚|伴侣/.test(t)) add('加入家庭互动的片段：一句话、一个场景或一个习惯')
    if (/旅行|旅途|出差|游/.test(t)) add('说明目的地、同伴、路线，以及让你印象深刻的瞬间')
    if (/困难|挫折|挑战|失败/.test(t)) add('指出最大的困难、你如何应对，以及最终结果')
    if (/选择|决定|决定/.test(t)) add('分享你当时的思考过程和其他备选方案')
  ...
  return (
    <main className="detroit-bg" style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <h1 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: '#1F1F1F', letterSpacing: '0.3px' }}>
                永恒<span style={{ color: '#2F3E5C' }}>档案</span>
              </h1>
              <p style={{ margin: '0px 0 0', fontSize: 8, color: '#6F6F6F', letterSpacing: '0.3px' }}>
                让记忆超越时间
              </p>
            </div>
          </div>
          {isLoggedIn && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'nowrap', minWidth: 0 }}>
              <div style={{
                ...
                <span style={{ color: '#2F3E5C', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100px', fontSize: 8 }}>{userEmail}</span>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {answerHistory.length > 0 && (
                <div className="glass-card" style={{ padding: 20, marginTop: 16 }}>
                  ...
                  >
                    <span>◈ 录音历史 ({answerHistory.length} 个版本)</span>
                    <span style={{
                        ...
                        <div key={session.id} style={{
                          ...
                          <div style={{
                            ...
                            <span style={{
                              ...
                              {idx === 0 ? '✨ 最新版本' : `版本 #${answerHistory.length - idx}`}
                            </span>
                            <span style={{
                              fontSize: 11,
                              color: '#94a3b8',
              ...
              <div style={{
                ...
                <p style={{ margin: '0 0 16px', fontSize: 13, opacity: outlineUnlocked ? 0.95 : 0.75, lineHeight: 1.5 }}>
                  {outlineUnlocked
                    ...
                    : `已完成 ${answeredSet.size} / ${MIN_ANSWERS_FOR_OUTLINE} 道题，达到 ${MIN_ANSWERS_FOR_OUTLINE} 题即可生成提纲。`}
                </p>
              ...
              <div style={{
                ...
                {totalCount === 0 && (
                  <div style={{
                    ...
                    题库未加载：请检查 Supabase RLS 策略是否允许读取默认题目（created_by 为 null 或等于当前用户），并在 SQL 编辑器执行 NOTIFY pgrst, 'reload schema' 以刷新缓存。
                  </div>
                )}
      ...
      <ConfirmDialog
        ...
        title="删除 Outline"
        message="确定要删除这个 Outline 吗？此操作无法撤销。"
      />
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <button
                    onClick={() => {
                      showToast('Premium checkout即将上线，请联系支持开通或使用兑换码', 'success')
                    }}
                    disabled={isPremium}
                    style={{
                      width: '100%',
