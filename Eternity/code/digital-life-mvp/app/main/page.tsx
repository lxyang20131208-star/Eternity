  ...
  return (
    ...
    >
      <div
        ...
        onClick={(e) => e.stopPropagation()}
      >
        <h3 style={{ margin: '0 0 8px', fontSize: 18, fontWeight: 600, color: '#2C2C2C' }}>
          {title}
        </h3>
        <p style={{ margin: '0 0 20px', fontSize: 14, color: '#5A4F43' }}>{message}</p>
...
function AudioPlayer({
  ...
  if (!audioUrl) {
    return <div style={{ fontSize: 12, color: '#5A4F43' }}>Failed to load audio</div>
  }
...
function MainPageContent() {
  // ===== 统一配色方案 - 与 draft 页面保持一致 =====
  // 统一暖色系配色方案
  const colors = {
    // 主色调
    primary: '#B89B72',        // 柔和暖棕
    primaryLight: '#E3D6C6',   // 浅米棕

    // 文字颜色
    text: '#3B2F23',           // 深暖灰
    textSecondary: '#8C8377',  // 柔和次要文字
    textMuted: '#B0A89C',      // 浅灰提示
    textWhite: '#FFFFFF',      // 白色

    // 背景色 (main page overrides to white)
    bg: 'white',
    bgWarm: 'white',
    bgAccent: 'white',
    bgCool: 'white',
  ...
  const MIN_ANSWERS_FOR_OUTLINE = 30

  // Feature unlock thresholds (based on answered_count, excluding Second Round Questions)
  const UNLOCK_THRESHOLDS = {
    photos: 10,
    tree: 20,
    outline: 30,      // Generate Outline + Review Outline
    places: 40,
    timeline: 50,
  ...
  // Helper function to check if a chapter is unlocked based on membership tier and payment count
  const isChapterUnlocked = (chapterName: string, tier: 'plus' | 'pro' | null, paymentCount: number) => {
    // Pro users have all chapters unlocked
    if (tier === 'pro') return true
    // Plus users unlock chapters based on payment count
    if (tier === 'plus') {
      const chapterIndex = CHAPTER_ORDER.indexOf(chapterName as typeof CHAPTER_ORDER[number])
      // If chapter is not in the predefined list, allow access
      if (chapterIndex === -1) return true
      // Chapter is unlocked if payment count > chapter index (0-indexed)
      return paymentCount > chapterIndex
    }
    // Non-premium users shouldn't reach here, but default to locked
    return false
  }
  ...
  function showLockedFeature(feature: string, requiredCount: number) {
    setLockModal({ show: true, feature, requiredCount })
  }
    ...
    try {
      ...
      if (userErr) {
        ...
        if (msg.toLowerCase().includes('auth session missing')) {
          ...
          setUserEmail(null)
          setProjectId(null)
          router.push('/signin?source=login')
          return
        }
        throw userErr
      }
  ...
  // Load premium status and tier from server (no client-side toggle)
  useEffect(() => {
    async function loadPremiumStatus() {
      if (DEV_FORCE_PREMIUM) {
        setIsPremium(true)
        setMembershipTier('pro') // default to pro for dev
        setPaymentCount(6) // pro users have all chapters
        return
      }
      if (!userId) {
        setIsPremium(false)
      ...
      try {
        // Fetch premium status and tier in parallel
        const [premiumRes, tierRes] = await Promise.all([
          supabase.rpc('is_premium', { p_user_id: userId }),
          supabase.rpc('get_membership_tier', { p_user_id: userId })
        ])

        if (premiumRes.error) throw premiumRes.error
        setIsPremium(Boolean(premiumRes.data))

        // Set tier (will be 'plus' or 'pro' if premium, null otherwise)
        if (!tierRes.error && tierRes.data) {
          const tier = tierRes.data as 'plus' | 'pro'
          setMembershipTier(tier)

          // For Plus users, fetch payment count from subscription metadata
          if (tier === 'plus') {
            ...
            // Get payment_count from metadata, default to 1 if not set
            const count = subData?.metadata?.payment_count ?? 1
            setPaymentCount(Math.min(Math.max(count, 1), 6)) // Clamp between 1-6
          } else {
            // Pro users have all chapters unlocked
            setPaymentCount(6)
          }
  ...
  function toggleChapter(name: string) {
    ...
    if (membershipTier === 'plus' && !isChapterUnlocked(name, membershipTier, paymentCount)) {
      const chapterIndex = CHAPTER_ORDER.indexOf(name as typeof CHAPTER_ORDER[number])
      if (chapterIndex !== -1) {
        setChapterLockModal({
          show: true,
          chapterName: name,
          requiredPayments: chapterIndex + 1
        })
        return
      }
    }
    ...
    return (
      <main className="detroit-bg" style={{
        ...
        <div className="glass-card" style={{
          ...
          <div style={{ color: '#5A4F43', fontSize: 14, letterSpacing: '0.5px' }}>
            SYSTEM INITIALIZING...
          </div>
        </div>
      </main>
  ...
  return (
    <main className="detroit-bg" style={{
          ...
          <div style={{
            ...
            <div className="glass-card" style={{
              ...
              <div style={{
                ...
                height: 80,
                margin: '0 auto 24px',
                background: 'linear-gradient(135deg, rgba(184,155,114,0.1), rgba(168,136,100,0.1))',
                border: '2px solid rgba(184,155,114,0.5)',
                borderRadius: 4,
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
              ...
              <div style={{ display: 'flex', gap: 16, flexDirection: 'column' }}>
                ...
                <Link
                  href="/checkout/"
                  className="cyber-btn"
                  style={{ borderRadius: 4, width: '100%', display: 'block', textDecoration: 'none' }}
                >
                  ◇ CREATE ACCOUNT
                </Link>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  {/* Question Navigation */}
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 12,
                        fontWeight: 600,
                        background: prevQuestion ? 'rgba(184,155,114,0.06)' : 'white',
                        color: prevQuestion ? '#8B7355' : '#5A4F43',
                        border: `1px solid ${prevQuestion ? 'rgba(184,155,114,0.3)' : '#E3D6C6'}`,
                        borderRadius: 4,
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 12,
                        fontWeight: 600,
                        background: nextQuestion ? 'rgba(184,155,114,0.06)' : 'white',
                        color: nextQuestion ? '#8B7355' : '#5A4F43',
                        border: `1px solid ${nextQuestion ? 'rgba(184,155,114,0.3)' : '#E3D6C6'}`,
                        borderRadius: 4,
              ...
              <div className="glass-card" style={{
                ...
                <div style={{
                  padding: 16,
                  background: 'white',
                  border: '1px solid rgba(90, 79, 67, 0.08)',
                  borderRadius: 8,
                  marginTop: 16,
                }}>
                  <div style={{
                    ...
                    <div style={{
                      ...
                      />
                        ...
                        <input
                          ...
                          style={{ display: 'none' }}
                        />
                        <span style={{ fontSize: 12, color: '#5A4F43', fontWeight: 500 }}>
                    ...
                    <div style={{
                      ...
                      {answerHistory.map((session, idx) => (
                        <div key={session.id} style={{
                          padding: 12,
                          background: idx === 0 ? 'rgba(184,155,114,0.05)' : 'rgba(227,214,198,0.06)',
                          border: idx === 0 ? '1px solid rgba(184,155,114,0.15)' : '1px solid rgba(227,214,198,0.6)',
                          borderRadius: 10,
                          display: 'flex',
                          flexDirection: 'column',
                          gap: 8,
                          ...
                          {session.transcript_text && (
                            <div style={{ marginTop: 4 }}>
                              {idx === 0 && editingTranscript === session.id ? (
                                ...
                                <div style={{
                                  padding: 10,
                                  background: idx === 0 ? 'rgba(184,155,114,0.03)' : 'white',
                                  border: idx === 0 ? '1px solid rgba(184,155,114,0.1)' : 'none',
                                  borderRadius: 8,
                                }}>
                                  <div style={{
                                    fontSize: 11,
                                    color: idx === 0 ? '#8B7355' : '#5A4F43',
                                    marginBottom: 6,
                                    fontWeight: 600,
                                    display: 'flex',
                                    justifyContent: 'space-between',
              ...
              {loadingAnswers ? (
                ...
                <div className="glass-card" style={{
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                        <div style={{
                          width: 20,
                          height: 20,
                          border: '3px solid rgba(34,34,34,0.12)',
                      ...
                      <div>
                        <button
                          ...
                          style={{
                            ...
                            fontSize: 14,
                            fontWeight: 600,
                            background: 'linear-gradient(135deg, rgba(184,155,114,0.15) 0%, rgba(168,136,100,0.15) 100%)',
                            color: '#8B7355',
                            border: '1px solid rgba(184,155,114,0.4)',
                            borderRadius: 4,
                            cursor: 'pointer',
                            transition: 'all 0.2s',
                            boxShadow: '0 0 15px rgba(184,155,114,0.15)',
                          }}
                        >
                          🚀 Generate Outline
                        </button>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <div>
                      <button
                        onClick={() => showLockedFeature('Generate Outline', UNLOCK_THRESHOLDS.outline)}
                        style={{
                          width: '100%',
                          padding: '12px 24px',
                          fontSize: 14,
                    ...
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                      <button
                        onClick={() => showLockedFeature('Review Outline', UNLOCK_THRESHOLDS.outline)}
                        style={{
                          padding: '10px 16px',
                          fontSize: 12,
                          fontWeight: 600,
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, minWidth: 0, flex: 1 }}>
                          <span style={{
                            ...
                            justifyContent: 'center',
                            borderRadius: 4,
                            background: chapterLocked ? 'rgba(150,150,150,0.2)' : 'rgba(184,155,114,0.1)',
                            border: chapterLocked ? '1px solid rgba(150,150,150,0.3)' : '1px solid rgba(184,155,114,0.3)',
                            color: chapterLocked ? '#999' : '#8B7355',
                            fontWeight: 700,
                            fontSize: 11,
                            flexShrink: 0,
                          }}>
                            {chapterLocked ? '🔒' : group.items.length}
                          </span>
                          <div style={{
                            fontSize: 13,
                            fontWeight: 600,
                            color: chapterLocked ? '#999' : '#222',
                            flex: 1,
                            lineHeight: 1.4,
                            textAlign: 'left',
                          }}>
                          ...
                          <div style={{
                            fontSize: 11,
                            color: chapterLocked ? '#999' : (answeredInGroup === group.items.length ? '#8B7355' : '#8B7355'),
                            fontWeight: 600,
                            fontFamily: 'monospace',
                            flexShrink: 0,
                          }}>
                            {chapterLocked ? '未解锁' : `[${answeredInGroup}/${group.items.length}]`}
                          </div>
                        </div>
                        <span style={{
                          transition: 'transform 0.2s',
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#5A4F43', marginBottom: 6 }}>
              Email
            </label>
            <input
              type="email"
              ...
              onFocus={(e) => (e.target.style.borderColor = '#B89B72')}
              onBlur={(e) => (e.target.style.borderColor = '#E3D6C6')}
            />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#5A4F43', marginBottom: 6 }}>
          ...
          {error && (
            <div style={{
              ...
              borderRadius: 8,
              fontSize: 12,
              color: error.includes('successfully') ? '#166534' : '#5A4F43',
            }}>
              {error}
            </div>
          )}
          <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                fontSize: 14,
                fontWeight: 600,
                background: authLoading || !authEmail || !authPassword ? '#EDE6DA' : 'linear-gradient(135deg, #B89B72, #A89070)',
                color: '#222',
                border: 'none',
                borderRadius: 8,
            ...
            {showAuthModal === 'signin' ? (
              <>
                ...
                <button
                  ...
                  style={{ background: 'none', border: 'none', color: '#B89B72', cursor: 'pointer', fontWeight: 600 }}
                >
                  Sign Up
                </button>
              </>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            {stylePrefs.authorStyle && AUTHOR_STYLES[stylePrefs.authorStyle] && (
              <p style={{ marginTop: 8, fontSize: 12, color: '#5A4F43', lineHeight: 1.5 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                fontSize: 14,
                fontWeight: 600,
                background: 'linear-gradient(135deg, #B89B72, #A89070)',
                color: '#222',
                border: 'none',
                borderRadius: 8,
                cursor: 'pointer',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {currentOutline.outline_json.sections.map((section, idx) => (
                  <div key={idx} style={{
                    ...
                    <h4 style={{ margin: '0 0 12px', fontSize: 16, fontWeight: 600, color: '#2C2C2C' }}>
                      {section.title}
                    </h4>
                    <ul style={{ margin: '0 0 12px', paddingLeft: 20, fontSize: 13, color: '#5A4F43', lineHeight: 1.6 }}>
                      {section.bullets.map((bullet, bidx) => (
                        <li key={bidx} style={{ marginBottom: 6 }}>{bullet}</li>
                      ))}
                    </ul>
                    {section.quotes && section.quotes.length > 0 && (
                      <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #E3D6C6' }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: '#5A4F43', marginBottom: 8, textTransform: 'uppercase' }}>
                          Notable Quotes
                        </div>
      ...
      >
        <div style={{ maxHeight: '60vh', overflowY: 'auto' }}>
          {allOutlines.length === 0 ? (
            <div style={{ padding: 24, textAlign: 'center', color: '#5A4F43' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {allOutlines.map((outline) => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    ...
                    <div style={{
                      ...
                      background: 
                        ...
                        outline.status === 'processing' ? '#dbeafe' : 'white',
                      color: 
                        outline.status === 'done' ? '#166534' : 
                        outline.status === 'failed' ? '#5A4F43' : 
                        outline.status === 'processing' ? '#2C2C2C' : '#5A4F43',
                    }}>
                      {outline.status.toUpperCase()}
                    </div>
                  </div>
                  <div style={{ fontSize: 12, color: '#5A4F43', marginBottom: 4 }}>
                  ...
                  {outline.error_text && (
                    <div style={{ fontSize: 11, color: '#5A4F43', marginTop: 8, padding: 8, background: '#fee2e2', borderRadius: 6 }}>
                      {outline.error_text}
                    </div>
                  )}
                  {/* Delete button */}
      ...
      {toast && (
        <div style={{
          ...
          color: toast.type === 'success' ? '#166534' : '#5A4F43',
          borderRadius: 12,
              ...
              {collaborators.map((collab) => (
                <div key={collab.id} style={{
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: 10, color: '#5A4F43' }}>{collab.role} • {new Date(collab.joined_at).toLocaleDateString()}</div>
                  </div>
                </div>
              ))}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{
              ...
              完成 <strong style={{ color: '#8B7355' }}>{lockModal.requiredCount}</strong> 道问题后即可解锁该功能
            </p>
            <div style={{
              ...
              <div style={{ fontSize: 24, fontWeight: 700, color: '#222' }}>
                {answeredSet.size} / {lockModal.requiredCount}
              </div>
              <div style={{
                height: 6,
                ...
                <div style={{
                  height: '100%',
                  width: `${Math.min(100, (answeredSet.size / lockModal.requiredCount) * 100)}%`,
      ...
      {/* Chapter Locked Modal for Plus Users */}
      {chapterLockModal?.show && (
        <div style={{
          position: 'fixed',
          top: 0,
          left: 0,
          ...
          justifyContent: 'center',
          zIndex: 1002,
        }} onClick={() => setChapterLockModal(null)}>
          <div style={{
            background: 'white',
            border: '2px solid #8B7355',
            borderRadius: 16,
            ...
            <p style={{
              ...
              {chapterLockModal.chapterName.replace(/^\d+\.\s*/, '')}
            </p>
            <p style={{
              margin: '0 0 24px',
              fontSize: 14,
              ...
              该章节需要 <strong>{chapterLockModal.requiredPayments}</strong> 个月付费才能解锁
            </p>
            <div style={{
              background: 'rgba(139, 115, 85, 0.1)',
              borderRadius: 8,
              ...
              <div style={{ fontSize: 12, color: '#8B7355', marginBottom: 4 }}>当前付费进度</div>
              <div style={{ fontSize: 24, fontWeight: 700, color: '#222' }}>
                {paymentCount} / {chapterLockModal.requiredPayments}
              </div>
              <div style={{
                height: 6,
                background: 'rgba(139, 115, 85, 0.2)',
                ...
                <div style={{
                  height: '100%',
                  width: `${Math.min(100, (paymentCount / chapterLockModal.requiredPayments) * 100)}%`,
                  background: 'linear-gradient(90deg, #8B7355, #A89070)',
                  borderRadius: 3,
                  transition: 'width 0.3s ease',
                }} />
